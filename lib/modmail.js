...
async function generalTicket(message, bot) {
  return new Promise(async function (resolve) {
    const channel = await utils.modmail.create(message.author, "general", "659089242785316865", bot);
...
async function reportTicket(message, bot) {
  return new Promise(async function (resolve, reject) {
    ...
    let proofLinkArray = [];
    const proofReqMessage = await message.author.send("Almost done! What type of proof are you going to supply? 📷 for local images/videos, 🔗 for remote images/videos (imgur.com/streamlabs.com) and 🧍 for witnesses.")
    ...
    await utils.modmail.fetch(message.author) // userID, channelID, archived, inquiryType
        ...
        .catch(async () => { // No ticket found \\
          ...
          if (message.channel.type === "dm") {
            ...
            const emoji = await utils.askReactions(confirmationMessage, ["🇦",/* "🇧",*/ "❌"], bot, 35000);
